...
3. `run_agent` resolves the current working directory and `DEVO_HOME`, loads the
   workspace-effective app config, then constructs the model catalog from the
   tracked directory and `providers.json` overlay before resolving the
   permission preset, provider Connection, and saved model entries.
...
5. The TUI starts with an `InitialTuiSession` containing the current session id,
   canonical model reference, provider wire API, reasoning effort selection,
   permission preset, and working directory.

`devo onboard` follows the same setup path, forces provider onboarding mode,
and exits after the provider Connection/model selection is saved. The CLI then prompts the
...
Provider configuration starts with the tracked template directory, then merges
user Connections and model records:

1. `devo-config` defines the canonical provider/model JSON and auth references.
2. `devo-core` resolves the effective provider Connection and model for the current workspace.
