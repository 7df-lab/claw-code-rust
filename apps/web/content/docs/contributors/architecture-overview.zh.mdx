...
3. `run_agent` 解析当前工作目录和 `DEVO_HOME`，先加载工作区 effective app config，再从 git 跟踪目录和 `providers.json` 覆盖项构建 model catalog，之后解析 permission preset、provider Connection 和 saved model entries。
4. `run_agent` 调用 `devo_tui::run_interactive_tui`。
5. TUI 以 `InitialTuiSession` 启动，其中包含当前 session id、规范 model reference、provider wire API、reasoning effort selection、permission preset 和 working directory。

`devo onboard` 使用同一条 setup 路径，但会强制进入 provider onboarding
mode，并在 provider Connection/model selection 保存后退出。CLI 随后提示用户运行 `devo`，
让下一个进程从新写入的配置启动。
...
Provider 配置从 git 跟踪的 template 目录开始，再合并用户 Connection 和 model record：

1. `devo-config` 定义规范 provider/model JSON 和 auth 引用。
2. `devo-core` 为当前工作区解析 effective provider Connection 和 model。
