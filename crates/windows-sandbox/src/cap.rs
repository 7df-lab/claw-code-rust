...
pub fn cap_sid_file(devo_home: &Path) -> PathBuf {
    devo_home.join("cap_sid")
}
...
pub fn load_or_create_cap_sids(devo_home: &Path) -> Result<CapSids> {
    let path = cap_sid_file(devo_home);
    if path.exists() {
...
pub fn workspace_cap_sid_for_cwd(devo_home: &Path, cwd: &Path) -> Result<String> {
    let path = cap_sid_file(devo_home);
    let mut caps = load_or_create_cap_sids(devo_home)?;
...
pub fn writable_root_cap_sid_for_path(devo_home: &Path, root: &Path) -> Result<String> {
    let path = cap_sid_file(devo_home);
    let mut caps = load_or_create_cap_sids(devo_home)?;
...
pub fn workspace_write_cap_sid_for_root(
    ...
    if canonical_path_key(root) == canonical_path_key(cwd) {
        workspace_cap_sid_for_cwd(devo_home, cwd)
    } else {
        writable_root_cap_sid_for_path(devo_home, root)
    }
}
...
mod tests {
    ...
    fn equivalent_cwd_spellings_share_workspace_sid_key() {
        ...
        let devo_home = temp.path().join("devo-home");
        std::fs::create_dir_all(&devo_home).expect("create devo home");
        ...
        let first_sid =
            workspace_cap_sid_for_cwd(&devo_home, canonical.as_path()).expect("first sid");
        let second_sid =
            workspace_cap_sid_for_cwd(&devo_home, alt_spelling.as_path()).expect("second sid");

        assert_eq!(first_sid, second_sid);

        let caps = load_or_create_cap_sids(&devo_home).expect("load caps");
    ...
    fn write_roots_get_path_scoped_sids() {
        ...
        let workspace_sid = workspace_write_cap_sid_for_root(&devo_home, &workspace, &workspace)
            .expect("workspace sid");
        let extra_sid = workspace_write_cap_sid_for_root(&devo_home, &workspace, &extra_root)
            .expect("extra root sid");

        assert_ne!(workspace_sid, extra_sid);
        assert_eq!(
            extra_sid,
            writable_root_cap_sid_for_path(&devo_home, &extra_root).expect("extra root sid again")
        );
