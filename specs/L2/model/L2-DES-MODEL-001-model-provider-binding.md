...
- `request_model`: provider-specific model identifier used in API calls.
...
`display_name` is mutable display metadata. It must not be used as a stable identifier, provider API model identifier, traceability key, or configuration reference. The stable references remain `binding_id`, `canonical_model_slug`, and `provider_id`; the provider request uses `request_model`. Runtime turn metadata keeps `model` as the catalog slug and `request_model` as the provider request model, even when both strings are identical.

Legacy configuration may provide `model_name` as a read alias for `request_model`. Writers must persist only `request_model` and remove the legacy key when updating the binding.
